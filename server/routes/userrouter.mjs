...
// PASSWORDS FOR EACH ADMIN ACCOUNT (as of 6/1/2025 11:24pm, passwords might change due to testing or demo)
// pres: passwordpresident vp: passwordvice teachers: passwordteachemily & passwordteachtan
// headadmin: passwordadmin, headingred: passwordingredient

//Google Calendar API Routes
//get calendar
router.get("/googcalendar/listevents", async (req,res) => {
    calendar.events.list({
        ...
        if (error) {
            ...
            if (result.data.items.length) {
                ...
                events.sort((a,b) => {
                    const date_a = new Date(a.start.dateTime || a.start.date)
                    const date_b = new Date(b.start.dateTime || b.start.date)
                    return date_a - date_b
                })
                // these docs again https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/sort
...
router.get("/googcalendar/getevent/:id", async (req,res) => {
    if (req.isAuthenticated()) {
        calendar.events.get({
            ...
            if (error) {
                ...
                if (result.data) {
                    ...
                    res.status(200).json({ message: 'No event with this id found.' });
                }
            }
        });
    } else {
...
router.post("/googcalendar/createevent", async (req, res) => {
    if (req.isAuthenticated()) {
        var event = {
            ...
            const check_start = new Date(event.start.dateTime).getTime()
            const check_end = new Date(event.end.dateTime).getTime()

            if (check_start > check_end) {
                res.status(400).json({ message: "The end time of the event cannot be before the start time of the event!" })
                return;
            }

            if ((check_start - check_end) == 0) {
                res.status(400).json({ message: "Events cannot end immediately right after they start!" })
                return;
            }

        const auth = new google.auth.GoogleAuth({
            keyFile: 'temasekcookingclub-7bfb3a28b621.json',
...
router.patch("/googcalendar/updateevent/:id", async (req,res) => {
    if (req.isAuthenticated()) {
        var event = {
            'summary': req.body.summary,
            'location': req.body.location,
...
//handle delete
router.delete("/googcalendar/deleteevent/:id", async (req,res) => {
    if (req.isAuthenticated()) {
        const auth = new google.auth.GoogleAuth({
            keyFile: 'temasekcookingclub-7bfb3a28b621.json',
            scopes: 'https://www.googleapis.com/auth/calendar',
        ...
        auth.getClient().then(a => {
            calendar.events.delete({
                ...
                console.log('Event deleted: %s', req.params.id);
